...
class MicroPackageManage
{
    ...
    public function getHomePackageList(){
		...
		$limitstr = '0,3';
        $data = $packetmodel->getPackageList($limitstr, 'p.id desc');
		if(empty($data)) return return_format('',90011,lang('90011'));
     ...
     * @return   array();
     */
    public function gotoPay($studentid,$ordernum,$paytype,$type=0){
        ...
        if($length == 1){
            ...
            switch ($paytype){
                ...
                case 2://微信支付
                    $wxpayobj = new Wxpay;
                    $notifyurl = config('param.server_url') . $this->wxnotifyurl;//异步回调地址
                    if($type == 1){//app支付
                        $result = $wxpayobj->appWxpay($ordernum, $subject, $amount, $body, $notifyurl);
            ...
            switch($mixtype){
                case 2:
                    ...
                    if($type == 1){//app支付
                        $result = $wxpayobj->appWxpay($ordernum, $subject,$price,$body, $notifyurl);
                        if ($result['result_code'] !== 'SUCCESS') {
                            return return_format('', 34034, lang('33031'));
                        }
                        $data['data'] = $result;
