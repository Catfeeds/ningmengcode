...
use Calendar;
class OfficalAppMyCourseManage
{
    protected $foo;
    protected $str;
    protected $array;
    ...
    public function getCategoryArr($organid)
    {
        ...
        if(empty($catelist)){
            return return_format('',33100,'该机构下没有分类');
        }
        //判断该机构添加的分类是那种类型
        $data['scene'] = 3;
    ...
    public function searchByCids($organid,$fatherid,$categoryid,$tagids,$pagenum,$limit){
        ...
        if(empty($category_id)){
            $data['schedulist'] = $schedumodel->getCourserAllList($limitstr);
            ...
            $data['schedulist'] = $schedumodel->getWebFilterCourserList($categoryids,$limitstr);
            $total = $schedumodel->getWebFilterCourserCount($categoryids);
        }
        //分页信息
        $data['pageinfo'] = [
    ...
    public function getCourseTags($organid){
        if(!is_intnum($organid)){
            return return_format('',33103,'参数错误');
        }
        $tagsmodel = new Coursetags;
        $tagarr = $tagsmodel->getTags($organid);
        if(empty($tagarr)){
            return return_format('',33104,'数据为空');
        }
    ...
    public function searchCourseByCname($organid,$search,$pagenum,$limit){
        ...
        if(empty($search)){
            return return_format('',33107,'请输入搜索内容');
        }
        //防止脚本攻击和SQL注入
        if (isset($search)){
        ...
        $coursearr = $schedumodel->getCourserListByCname($str,$limitstr);
        $total = $schedumodel->getCourserListCount($str);
        //分页信息
        $data['pageinfo'] = [
            'pagesize'=>$limit ,// 每页多少条记录
            'pagenum' =>$pagenum ,//当前页码
        ...
        if(empty($coursearr)){
            return return_format($coursearr,0,'没有数据');
        }else{
    ...
    public function getMyClass($studentid, $organid, $pagenum, $limit)
    {
        ...
        if(empty($orderlist)){
            return return_format([],33100,'没有数据');
        }
    ...
    public function getReserveStatus($studentid){
        ...
        $data['count'] = $count;
        if($count <0){
            return return_format('',33300,'查询失败');
        }else{
            return return_format($data,0,'查询成功');
        }
    }
    ...
    public function getReserveClass($studentid,$organid,$pagenum,$limit){
        ...
        //实例化model
        $ordermodel = new Ordermanage;
        $orderlist = $ordermodel->getReserveClass($studentid, $limitstr, $this->orderstatus);
    ...
    public function getReserveLessons($studentid,$organid,$schedulingid,$ordernum){
        ...
        if (empty($unitlist)) {
            return return_format($this->foo, 33004, '数据为空');
        }
        //获取课节信息
        $count = 0;
    ...
    public function getBuyList($studentid, $organid, $pagenum,$limit)
    {
        ...
        foreach ($orderlist as $k => $v) {
            //如果是一对一课程，查看是否预约完毕；1为预科完毕
            if ($v['type'] == $this->type) {
                $orderlist['reserve'] = $this->reserveLearned($v['schedulingid'], $studentid,$v['ordernum']);
            }
    ...
    public function getClassSchedule($studentid,$organid,$schedulingid,$ordernum)
    {
        if(!is_intnum($schedulingid) || !is_intnum($organid)){
            return return_format('',35001,'参数错误');
        }
        //实例化model
        if(!is_intnum($schedulingid) || !is_intnum($organid)){
        ...
        //查询出所有课时
        $lessonarr = $lessonsmodel->getLessonsByscheduid($schedulingid,$organid);
        $lesscount  = count($lessonarr);
        //返回结果集
        $courserinfo['unit'] = $unitlist;
        $courserinfo['lesscount'] = $lesscount;
    ...
    public function getFreeList($organid, $teacherid, $date)
    {
        ...
        //返回老师可约课的时间
        if (empty($weeklist)) {
            return return_format($weeklist, 33007, '没有可预约的时间');
        }
        return return_format($weeklist, 0, '查询成功');
...
/*        $configmodel = new Organconfig;
        ...
        $freearray = array_column($freearray,'timekey');
    ...
    public function insertComment($data){
        if(!is_intnum($data['curriculumid']) || !is_intnum($data['classtype']) || !is_intnum($data['studentid']) ||!is_intnum($data['allaccountid']) ||!is_intnum($data['score']) ||!is_intnum($data['schedulingid']) ||!is_intnum($data['organid']) ){
        ...
        if($id){
            ...
            return return_format('',33016,'添加失败');
        }
    }
    /**
     * 计算一对一排课是否预约完成
