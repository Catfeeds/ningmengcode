...
use Login;
class OfficalAppHomepageManage
{
    protected $foo;
    protected $str;
    protected $array;
     ...
     *
     */
    public function getCategoryList($organid,$studentid=null)
    {
        //查看学生是否登陆定制分类,没有就默认分类
        $loginobj = new Login;
        /* $studentid = $loginobj->checkIsLogin(1);*/
        if (!empty($studentid)) {
            $studentmodle = new Studentinfo;
            $studentinfo = $studentmodle->getStudentInfo($studentid);
        }
        $schedumodel = new Scheduling;
        $catemodel = new Category;

        if (empty($studentinfo['favorcategory'])) {
            //查询排序靠前的三个一级分类
            $category = $catemodel->getTopList($organid);
            $list = array_slice($category, 0, 3);
            foreach ($list as $k => $v) {
                $categorylist = $catemodel->get_category($v['category_id']);
                $categoryids = rtrim($categorylist, ',');
                $list[$k]['data'] = $schedumodel->getCateCourserList($organid, $categoryids);
            }
        } else {
            $list = explode(',', $studentinfo['favorcategory']);
            foreach ($list as &$v) {
                $v = $catemodel->getCategoryBycid($v);
                $categorylist = $catemodel->get_category($v['category_id']);
                $categoryids = rtrim($categorylist, ',');
                $v['data'] = $schedumodel->getCateCourserList($organid, $categoryids);
            }

        }
        if (empty($list)) {
            return return_format($list, 31001, '数据为空');
        } else {
    ...
    public function getScheduList ($organid)
    {
        ...
        if(empty($teacherlist)){
            return return_format($teacherlist,31003,'没有数据');
        }else{
    ...
    public function getSlideList($organid)
    {
        ...
        if(empty($res)){
            return return_format([],31005,'没有数据');
        }
            return return_format($res,0,'查询成功');
    }
    /**
