...
class AppTeacherManage
{
    ...
    public function getTeacherList()
    {
        ...
        if(empty($teacherlist)){
            ...
            $teacherlist = array_chunk($teacherlist,3);
     ...
     *
     */
    public function getRecommendTeacherList()
    {
        $teachermodel = new Teacherinfo;
        $teacherlist = $teachermodel->getRecommendList();
        foreach($teacherlist as $k=>$v){
            unset($teacherlist[$k]['profile']);
            unset($teacherlist[$k]['classesnum']);
            unset($teacherlist[$k]['recommend']);
            unset($teacherlist[$k]['identphoto']);
            unset($teacherlist[$k]['slogan']);
        }
        return return_format($teacherlist,0,lang('success'));
    }
    /**
    ...
    public function getTeacherData($teacherid)
    {
        ...
        if(empty($baseinfo)){
            return return_format([],36002,lang('36002'));
        }
        //查看是否收藏
        //查看是否登录
