...
class TeacherLable extends Model
{
	 ...
	 */
    public function getLableList($start=0,$offset=5)
    {
    	//分页兼容
    	$limit = $start>0 ? $start.','.$offset : $offset ;
    	//获取字段
    	$field = 'id,tagname,addtime,fatherid,status' ;
        return Db::table($this->table)->where(['fatherid'=>0,'delflag'=>0])->field($field)->limit($limit)->select();
    }
    /**
	 * 根据机构id 统计标签数量
	 * @Author wyx
	 ...
	 * 
	 */
	public function addLable($tagname){
	 ...
	 * 
	 */
	public function deleteLable($lableid){
	 ...
	 */
	public function getValueList($lableid,$start=0,$offset=5)
    {
    	//分页兼容
    	$limit = $start>0 ? $start.','.$offset : $offset ;
    	//获取字段
    	$field = 'id,tagname,addtime,fatherid,status' ;
        return Db::table($this->table)->where(['fatherid'=>$lableid,'delflag'=>0])->field($field)->limit($limit)->select();
	 ...
	 * 
	 */
	public function deleteLableValue($lableid){
		$where  = [
					'id'=> $lableid,
				] ;
