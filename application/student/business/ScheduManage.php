...
class ScheduManage
{
    ...
    public function getCateLeader($cateid,$department=' > '){
        ...
        //交换教师 排位序号
        $resultarr =  $coursemodel->getCategoryList();
        $floor1[0] = [
            'id' => 0  ,
            'title' => '不限' ,
            'rank' => 0 ,
        ...
        if( !empty($linecate['rank']) ){
            ...
            if( isset($linecate['rank']) && $linecate['rank'] == 1 ){//选中一级情况
                ...
                $leader[0] = [ 'categoryname' => $floor1[$cateid]['title'] ,'category_id'=>$cateid ] ;
                ...
                return return_format(['catetree'=>$floor1,'leader'=>$leader],0) ;
                ...
                $leader[0] = [ 'categoryname' => $floor1[$cateid1]['title'] ,'category_id'=>$cateid1 ] ;
                ...
                foreach ($floor2 as $val) {
                    if( isset($floor1[$val['fid']]) && $cateid1==$val['fid'] ){
                        if($cateid==$val['id']){
                            ...
                            $leader[1] = [ 'categoryname' => $val['title'] ,'category_id'=>$cateid ] ;
                        }
                ...
                return return_format(['catetree'=>$floor1,'leader'=>$leader],0) ;//implode($department, $leader)
            }else{// 选中三级情况
                $cateid1 = $cateids[0] ;//一层 选中的id
                ...
                foreach ($floor3 as $val) {
                    if( isset($floor2[$val['fid']]) && $cateid2==$val['fid'] ){
                        if($cateid==$val['id']){
                            ...
                            $leader[2] = [ 'categoryname' => $val['title'] ,'category_id'=> $cateid ] ;
                        }
                ...
                foreach ($floor2 as $val) {
                    if( isset($floor1[$val['fid']]) && $cateid1==$val['fid'] ){
                        if($cateid2==$val['id']){
                            $val['selected'] = true ;//选中二级分类
                            //$leader[2] = $val['title'] ;// 导航拼接
                            $leader[1] = [ 'categoryname' => $val['title'] ,'category_id'=> $cateid2 ] ;
                        }
                        array_push($floor1[$cateid1]['children'],$val);
                    }
                }
            ...
            if($cateid == 0){
                ...
                $floor1 = $this->dealArrayStruct($floor1);
                return return_format(['catetree'=>$floor1,'leader'=>[]],0);
            }else{
                $floor1 = $this->dealArrayStruct($floor1);
                return return_format(['catetree'=>$floor1,'leader'=>[]],0);
            }
    ...
    public function  getCurriculumInfo($courseid,$teacherid,$date,$fullpeople)
    {
        ...
        if(empty($courseinfo)){
            return return_format('',39000,'该课程已经删除');
        }
     ...
     * @return   array();
     */
    public function chooseAllList($courseid)
    {
        if(!is_intnum($courseid)){
            return return_format($this->str,35000,lang('35000'));
        }
        ...
        $field = 's.id as scheduid,s.gradename,s.fullpeople,s.starttime,s.teacherid';
        $result = $model->getScheduList($where,$field);
        return return_format($result,0,lang('success'));
    }
        ...
        foreach($scheduarr as $k=>$v){
            ...
            if ($schedustatus['status'] == 1 && ($schedustatus['classstatus'] == 0 || $schedustatus['classstatus'] == 1)) {
                if($count >0){
                    ...
                    $applewhere = [
                        'newlessonsid' => $v['scheduid'],
                        'status' => 1
                    ];
                    $scheducount = $applyschedu->getCourseCount($applewhere);
                    if($scheducount>0){
                        $scheduarr[$k]['applystatus'] = 2;
                    }
