    ...
    */
    ...
    public function addEdit($data){
        ...
        if(isset($data['id'])){
            ...
            //添加
            $data = where_filter($data,array('curriculumid','unitname','unitsort','schedulingid'));
            $ids = Db::name($this->table)->insertGetId($data);
        }
        return $ids?array('code'=>0,'info'=>$ids):array('code'=>10110,'info'=>'操作失败');
    }
