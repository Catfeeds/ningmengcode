...
class AlipayTradeService {
	 ...
	 */
	function writeLog($text,$filename='') {
		// $text=iconv("GBK", "UTF-8//IGNORE", $text);
		//$text = characet ( $text );
		$path = dirname ( __FILE__ ).DIRECTORY_SEPARATOR."./../../runtime/log/" ;
		if(empty($filename)) $filename ="alipaylog.txt" ;
		//日志记录
		file_put_contents ( $path.$filename, date ( "Y-m-d H:i:s" ) . "  " . $text . "\r\n", FILE_APPEND );
	}
}
